...
def index(request):
    most_popular_posts = Post.objects.annotate(Count('likes')).prefetch_related('author').order_by('-likes__count')
    ...
    most_fresh_posts = Post.objects.all().prefetch_related('author').order_by('-published_at')
